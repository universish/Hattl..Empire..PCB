...
Author: Saffet Yavuz (universish)
...
class KiCadAiCopilotPlugin(pcbnew.ActionPlugin):
    ...
    def check_cuda_toolkit(self, gpu_backend):
        """
        Validates the presence of NVIDIA CUDA Toolkit if the user selects a CUDA-based backend.
        Offers installation guidance if not found.
        """
        if "cuda" not in gpu_backend.lower():
            return True # Vulkan/wgpu doesn't need CUDA Toolkit

        # Simple check for nvcc in PATH
        from shutil import which
        if which("nvcc") is None:
            if 'wx' in sys.modules:
                msg = (
                    f"You selected '{gpu_backend}' as the GPU backend, but NVIDIA CUDA Toolkit (nvcc) is missing.\n\n"
                    "For Linux (Fedora/Ubuntu): sudo dnf install cuda / sudo apt install nvidia-cuda-toolkit\n"
                    "For Windows: Download from developer.nvidia.com/cuda-downloads\n\n"
                    "Falling back to Vulkan (wgpu) for this session."
                )
                wx.MessageBox(msg, "CUDA Toolkit Missing", wx.ICON_WARNING)
            else:
                pcbnew.wxLogMessage("CUDA Toolkit missing, falling back to Vulkan (wgpu).")
            return False
        return True

    def extract_board_state(self, board, config):
        # Determine layer count
        layer_count = board.GetCopperLayerCount()

        gpu_backend = config.get("gpu_backend", "vulkan_wgpu")
        if not self.check_cuda_toolkit(gpu_backend):
            gpu_backend = "vulkan_wgpu"
...
'''
    ...
    def Run(self):
        try:
            ...
            # Read Config
            config_path = os.path.join(os.path.dirname(__file__), "config.json")
            config = {"gpu_backend": "vulkan_wgpu"}
            if os.path.exists(config_path):
                try:
                    with open(config_path, 'r') as f:
            ...
            # 2. Extract State
            state = self.extract_board_state(board, config)
